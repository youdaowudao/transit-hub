...
状态：待开发。本方案以 `01-SPEC.md` 为唯一需求边界。
...
3. 多分组、重复 Key、重复账号等无法确定归属的情况标为 `unallocatable`；缺 Key、成员关系变化、主站或成本查询失败、缺成本项等标为 `failed`。每项都追加包含 `connectionId` 的问题。
...
1. 全部 expected 连接闭合时，返回全局 `exact`、`totalCost` 和 `totalProfit`；全局成本、利润分别为正式分组成本、利润之和。
2. 否则全局状态为 `partial` 或 `unavailable`，`totalCost` 和 `totalProfit` 均为 `null`；不因某些精确分组存在而伪造总利润。
