    ...
    connectionHealth: {
      ...
      prioritySync: {
        ...
        reasons: {
          ...
          priority_batch_in_progress: '本轮 Priority 正在分批写回',
          priority_snapshot_stale: 'Priority checkpoint 已领先当前快照，等待重新校验',
      ...
      safety: {
        ...
        observationCountHint: '允许 3-5 次，包含第一次失败；次数越少，确认越快，但抵抗瞬时故障误判的余量也越小。',
        jitter: '确认抖动（秒）',
        ...
        delays: '确认间隔（秒）',
        delaysHint: '每项会在上一次探测响应返回后开始等待；总耗时还包含每次最长 10 秒的真实探测和队列等待，请按期望完成时间设置较小值。',
      ...
      policyDrawer: {
        ...
        autoDegradeHelp: '开启后，探活结果会推进本地健康状态；Sub2API 的自动停用必须经过安全闸门确认，不由旧失败阈值直接触发。完整响应超过 5000 ms 但未超过 10 秒时记为 slow_response，并按高延迟状态处理。',
        ...
        tooltips: {
          ...
          failureThreshold: '当前 Sub2API 主站的自动停用不使用此阈值，而由安全闸门中的确认观察次数、确认间隔、共同故障熔断和存活底线共同裁决；此字段仅保留旧策略兼容。',
