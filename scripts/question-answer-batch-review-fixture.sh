...
SQL_FILE="$ROOT_DIR/backend/internal/modules/connection_health/testdata/task2_question_answer_batch_review_browser_fixture.sql"
ACTION="${1:-}"

case "$ACTION" in
  prepare|count|cleanup) ;;
  *)
    echo "用法: bash scripts/question-answer-batch-review-fixture.sh prepare|count|cleanup" >&2
    exit 2
    ;;
esac

for required_name in TASK2_BROWSER_DATABASE_URL TASK2_BROWSER_USER_ID TASK2_BROWSER_TARGET_ID; do
...
case "$TASK2_BROWSER_DATABASE_URL" in
  postgres://*|postgresql://*) ;;
  *)
    echo "TASK2_BROWSER_DATABASE_URL 必须是 PostgreSQL URL" >&2
    exit 2
    ;;
esac

database_authority="${TASK2_BROWSER_DATABASE_URL#*://}"
...
if [[ "$TASK2_BROWSER_DATABASE_URL" == *\?* ]]; then
  database_query="${TASK2_BROWSER_DATABASE_URL#*\?}"
...
if ! actual_server_addr="$(
  "${sanitized_psql[@]}" "$TASK2_BROWSER_DATABASE_URL" \
...
"${sanitized_psql[@]}" "$TASK2_BROWSER_DATABASE_URL" \
  -X -Atq -v ON_ERROR_STOP=1 \
  -v "user_id=$TASK2_BROWSER_USER_ID" \
  -v "target_id=$TASK2_BROWSER_TARGET_ID" \
  -v "prepare=$prepare" \
  -v "count=$count" \
  -v "cleanup=$cleanup" \
  -f "$SQL_FILE"
