...
run_connection_health_gate() {
  ...
  run_step "frontend connection health regression tests" \
    bash -c "cd '$ROOT_DIR/frontend' && npm run test -- connection-health-async-priority.test.ts connection-health-main-site-error.test.ts connection-health-main-site-error.behavior.test.ts connection-health-multiplier-resolution.test.ts connection-health-production-rank.test.ts connection-health-question-answer.test.ts connection-health-question-answer.behavior.test.ts connection-health-question-answer-repeat-queue.behavior.test.ts connection-health-question-keywords.behavior.test.ts connection-health-today-accuracy.behavior.test.ts connection-health-refresh-coordinator.test.ts connection-health-refresh-flow.test.ts use-connection-health-race.test.ts"
